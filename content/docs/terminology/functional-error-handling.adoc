...
_Функциональный (типизированный) подход к обработке ошибок — способ сигнализировать об ошибках через тип результата (контейнер/ADT), а не через исключения._

В этом подходе обработка ошибок заключается в том, чтобы при проектировании не тотальных^*^ функций, для сигнализации об ошибках использовать не исключения, а тип-контейнер в качестве результата функции, который может содержать в себе либо результат успешного выполнения, либо ошибку.
